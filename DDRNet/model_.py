...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, planes=64, spp_planes=128, head_planes=128, augment=True):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(14, planes, kernel_size=3, stride=2, padding=1),
    ...
    def forward(self, x):
        ...
        x = self.layer2(self.relu(x))  #2,128,28,28
        layers.append(x)

        x = self.layer3_1(self.relu(x))  #2,256,14,14
        layers.append(x)
        x_ = self.layer3_1_(self.relu(layers[1])) #2,128,28,28
        x = x + self.down3_1(self.relu(x_))
        x_ = x_ + F.interpolate(
            self.compression3_1(self.relu(layers[2])),
            size=[height_output, width_output],
            mode='bilinear',align_corners=True)  #2,128,28,28

        x = self.layer3_2(self.relu(x))  #2,128,14,14
        layers.append(x)
        x_ = self.layer3_2_(self.relu(x_))
        x = x + self.down3_2(self.relu(x_))  #2,128,14,14
        x_ = x_ + F.interpolate(
        ...
        x = self.layer4(self.relu(x))  #2,128,7,7
        layers.append(x)
        x_ = self.layer4_(self.relu(x_))
        x = x + self.down4(self.relu(x_))  #2,128,7,7
        x_ = x_ + F.interpolate(
        ...
        x_ = self.final_layer(x + x_)  #2,128,28,28
...
def DualResNet_imagenet(pretrained=False):
    model = DualResNet(BasicBlock, [3, 4, 6, 3], num_classes=2, planes=64, spp_planes=128, head_planes=256,
                       augment=True)
    if pretrained:
...
if __name__ == '__main__':
    model = get_seg_model(1).cuda()
    x = torch.zeros(2, 14, 224, 224).cuda()
    outs = model(x)
    for y in outs:
        print(y.shape)
